<?php
	...
	$_total_products = $this->tpp_getTotalProducts( $_from, $_to, $category_id, 1, 0 );
	...
	if( ( $all_pg ) >= $cur_all_pg + 2 ) {
		?>
			<div class="clr"></div>
			<div class='ik-product-load-more' align="center" onclick='loadMoreProducts("<?php echo esc_js( $params["from"] ); ?>","<?php echo esc_js( $params["to"] ); ?>","<?php echo $_limit_start+$_limit_end; ?>","<?php echo esc_js( $params["vcode"]."-".$params["from"]."-".$params["to"] ); ?>","<?php echo $_total; ?>",request_obj_<?php echo esc_js( $params["vcode"] ); ?>)'>
